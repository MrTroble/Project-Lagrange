...
class TGAppIO : public tge::io::IOModule {
 public:
  ...
  std::vector<tge::graphics::TDataHolder> bindings;
  ...
  TGAppIO() { calculateMatrix(); }

  void calculateMatrix() {
    projectionMatrix =
        glm::perspective(glm::radians(45.0f), aspectRatio, 0.01f, 10000.0f);
    ...
    mvpMatrix = projectionMatrix * view *
                (glm::translate(translation) * rotation * glm::scale(scale) *
                 glm::translate(implTrans));
    mvp2Matrix = projectionMatrix * view *
                 (glm::translate(translation + glm::vec3(0, 0, 0.001)) *
                  rotation * glm::scale(scale) * glm::translate(implTrans));
  }

  void sendChanges() {
    using namespace tge::graphics;
    const std::array changeInfos = {
        BufferChange{this->bindings[0], &this->mvpMatrix,
                     sizeof(this->mvpMatrix)},
        BufferChange{this->bindings[1], &this->mvp2Matrix,
                     sizeof(this->mvp2Matrix)}};
    this->api->changeData(changeInfos);
  }

  void tick(double deltatime) override {
    const auto step = deltatime * 10;
    if (sState || wState) translation.x += sState ? -step : step;
    if (aState || dState) translation.y += aState ? -step : step;
    if (qState || eState) translation.z += eState ? -step : step;
    wState = false;
    aState = false;
    sState = false;
    dState = false;
  ...
  void mouseEvent(const tge::io::MouseEvent& event) override {
    if (!isFocused()) {
      if ((event.pressed & 1) == 1) {
        ...
        scale = glm::clamp(scale, glm::vec3(0.01f, 0.01f, 0.01f),
